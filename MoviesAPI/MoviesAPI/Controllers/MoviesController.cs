...
namespace MoviesAPI.Controllers
{
    ...
    public class MoviesController : ControllerBase
    {
        ...
        public async Task<ActionResult<MoviePutGetDTO>> PutGet(int id)
        {
            ...
            var genreNonSelectedIds = await context.Genres
                .Where(x => !genreSelectedIds.Contains(x.Id))
                .ToListAsync();

            var movieTheatersIds = movie.MovieTheaters.Select(x => x.Id).ToList();
            var nonSelectedTheaters = await context.MovieTheaters
                .Where(x => !movieTheatersIds.Contains(x.Id))
                .ToListAsync();

            var nonSelectedGenreDTOs = mapper.Map<List<GenreDTO>>(genreNonSelectedIds);
            var nonSelectedMovieTheaterDTOs = mapper.Map<List<MovieTheaterDTO>>(nonSelectedTheaters);
            ...
            response.NonSelectedMovieTheaters = nonSelectedMovieTheaterDTOs;
        ...
        public async Task<ActionResult> Put(int id, [FromForm] MovieCreationDTO movieCreationDTO)
        {
            ...
            if (movieCreationDTO.Poster != null)
            {
                movie.Poster = await fileStorageService
